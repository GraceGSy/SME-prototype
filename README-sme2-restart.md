...
fresh Stage 4 graph. Bidirectional matches went from 12 to **14** — the fix changed two real
outcomes, not just the case that motivated it:
...
**Generated by `ClosestMatchGraph.fan_in_correspondence_table()` + `fan_in_table.
render_correspondence_table_markdown()`**, not hand-transcribed — see `pipeline/
closest_match_graph.py` for the method. Same compact rendering as the paragraph-level table
below: two columns, each cell listing every unit from that paper participating in the row.
**Bold** marks a unit that belongs to a genuinely bidirectional (both-directions-confirmed)
node; unbolded entries are non-redundant one-directional claimants (already filtered through
`redundant_edges()`, same discipline as Step 2 above) — a plain, un-bolded entry means it
independently pointed at this target but was never reciprocated back.

Unlike the version of this table shown earlier in this conversation, this one also surfaces
rows with **no bidirectional partner at all** — a node two or more units from the other paper
both independently chose as their closest match, but whose own pick (if any) landed elsewhere
or nowhere. That's the "even unrequited" half of this table's own name, and the hand-built
version of this table missed it entirely: **14 of the 18 rows below are bidirectional; the
last 4 are fully unrequited** (no bold anywhere in the row) — e.g. two distinct corpusstudio
Related Work subsections (`Finding Relevant Examples`, `Community Writing Norms`) both
independently claiming examplore_chi18's `RELATED WORK > Learning APIs with Code Examples`,
which itself picked something else:
...
| RELATED WORK > Learning APIs with Code Examples | Background and Related Work > Finding Relevant Examples, Background and Related Work > Community Writing Norms |
| RELATED WORK > Learning APIs with Code Examples, RELATED WORK > Mining and Visualization of API Usage, RELATED WORK > Interfaces for Exploring Collections of Complex Objects | Background and Related Work > Writing with External Text |
| SYSTEM ARCHITECTURE AND IMPLEMENTATION (whole) | Corpus Studio > Implementation Details, Data & Processing (whole) |
| SYSTEM ARCHITECTURE AND IMPLEMENTATION > Data Collection | Data & Processing > Embedding Process, Data & Processing > Pre-processing Documents |

Row order (a presentation choice the method itself leaves to the caller) follows
examplore_chi18's own document order for the 14 bidirectional rows, then the 4 fully-unrequited
rows at the end, since they have no bidirectional member to anchor a document-order position.
The `(whole)`/no-`(whole)` distinction is read per-paper from each unit's actual `subsections`
presence anywhere in the graph (e.g. examplore_chi18's `CONCLUSION` and `DISCUSSION AND
LIMITATIONS` genuinely have no subsections, so neither gets a `(whole)` tag, even though
corpusstudio's `Discussion` does) — computed entirely from the graph's own contents, no
re-reading of the source paper JSON required.
...
edge and the Study Procedure/Methodology "near-miss") are now stale, and the 4 newly-surfaced
unrequited rows above aren't reflected in that narrative at all.
...
**Rerun 2026-08-29 after the role-vs-content prompt fix** to `closest_paragraph_match_within_section.py`'s
own `SYSTEM_PROMPT` (the paragraph-level analog of the `closest_section_match_batch.py` fix — see
Stage 5's own notes). Both directional Stage 5 passes for the Introduction↔Introduction pairing were
rerun and fed into a fresh `ParagraphMatchGraph`. Bidirectional matches went from 4 to **5** —
`corpusstudio ¶0` ↔ `examplore_chi18 ¶0` newly confirms; previously corpusstudio ¶0's own pick was
**null**, reciprocating neither of the two examplore_chi18 paragraphs (¶0, ¶1) claiming it. `fan_in_groups`
is still 3, but that group's shape changed along with it: examplore_chi18 ¶1 is now the sole unreciprocated
claimant on corpusstudio ¶0, rather than ¶0 and ¶1 both being stranded together.

**Generated by `ParagraphMatchGraph.fan_in_correspondence_table()` + `fan_in_table.
render_correspondence_table_markdown()`**, not hand-transcribed — same method family as the
section-level table above, one granularity down: two columns, each cell listing every paragraph from
that paper participating in the row, sorted by paragraph number ascending. **Bold** marks a paragraph
that belongs to a genuinely bidirectional node; unbolded entries are one-directional-only claimants.
Unlike the section-level sibling, there is no `redundant_edges()` filtering step here (see this
method's own docstring, and Stage 6's "no `redundant_edges()` on this class" note, for why):
...
**The "Worked example" prose and 3-column table above this point** (the "4 confirmed bidirectional
matches" / "3 `fan_in_candidates()` groups" narrative, and the "Combined view" table) **still reflect
the pre-fix run** and haven't been updated to match this rerun — the confirmed-match count, and the
description of ¶0/¶1's fan-in group, are now both stale. Worth a follow-up pass if that narrative is
still being relied on.

Confirmed end-to-end: `add_section_pairing`'s stats, `one_to_many_candidates()`,
`fan_in_candidates()`, `fan_in_correspondence_table()`, and `save`/`load` round-tripping all check
out against this real (rerun) pair.
