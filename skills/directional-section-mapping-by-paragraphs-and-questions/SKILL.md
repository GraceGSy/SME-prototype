...
description: "Given two sections-with-paragraphs-and-questions.json files (from \"annotate-section-questions-given-paragraphs\" — each entry has section_name, section_number, a full paragraphs array, and question_this_section_answers), maps every section of \"paper1\" onto its closest section in \"paper2\" using both the paragraph text and the precomputed question together as joint, co-equal evidence — no PDF is opened. For a no-match entry, pulls paper1's own question_this_section_answers forward verbatim rather than leaving it null or composing a fresh one (fixed 2026-08-16). Outputs p1-p2-section-mapping-by-paragraphs-and-questions.json. Use when both papers already have paragraph-and-question files and the user wants a directional mapping richer than question-only matching but without re-reading either PDF. For question-field-only matching use \"directional-section-mapping-by-questions-only\"; for reading PDFs directly use \"directional-section-mapping.\""
---
