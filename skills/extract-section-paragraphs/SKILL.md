...
- **Identify a bare numbered (or lettered) sub-heading with no prose of its own, and merge it FORWARD into the paragraph that follows — added 2026-08-27.** A line matching a decimal-numbered (or lettered) heading pattern one or more levels deeper than the enclosing section — e.g. `4.1.5 Additional Features for Context and Familiarity.` sitting inside section `4`/subsection `4.1` — is a real structural sub-heading, not body prose. Unlike a caption (which is stripped or merged backward into what precedes it), a bare sub-heading like this should be merged **forward**, onto the front of the very next paragraph's text, as a short lead-in label — never deleted outright, and never left as its own standalone `paragraph_number`. Identify it by content, the same discipline as caption/chart-label detection: a short line (title-length, not a full sentence — it may still end in a period as part of its own title, as in the example above) that is immediately followed by a paragraph break with **no** prose of its own belonging to the heading before that break.
  - **Distinguish this from the paper's own ordinary labeled-paragraph-opening convention, which should be left completely alone.** Many papers open a paragraph with a bolded or numbered label directly followed, in the *same* paragraph, by a full sentence continuing the thought — e.g. `Within-structure alignment. AbstractExplorer also aligns the sentences in three different ways...` or `4.1.1 Slicing abstracts for... We first split each abstract into...`. That is normal prose, not a bare heading, and must not be touched. The test: does real explanatory prose begin in the *same* paragraph as the label, or does the label sit completely alone, with the first real sentence only starting in the *next* extracted paragraph? Only the second case is the bug this rule fixes.
  - Caught via a real example: `4.1.5 Additional Features for Context and Familiarity.` was extracted as its own zero-content paragraph, sandwiched between two unrelated, fully-formed paragraphs (`Within-structure alignment. ...` before it, `Abstract and TLDR panel. Sentence-level reading may be new to readers...` after it). The heading has no lead-in sentence of its own before its first labeled sub-item begins, so the correct output merges it onto the front of the `Abstract and TLDR panel.` paragraph (`"4.1.5 Additional Features for Context and Familiarity. Abstract and TLDR panel. Sentence-level reading may be new to readers..."`) rather than leaving it as a standalone paragraph — this was found and fixed as part of a corpus-wide audit, 2026-08-27.
...
- **Leaving a bare numbered/lettered sub-heading as its own standalone `paragraph_number`, with no prose of its own — added 2026-08-27.** A line like `4.1.5 Additional Features for Context and Familiarity.` with nothing else in its own paragraph unit is a real sub-heading, not a content-less artifact to strip and not a normal paragraph to leave alone — merge it forward onto the front of the paragraph that follows, and renumber. Don't confuse this with the paper's own ordinary labeled-paragraph-opening style (a label followed by real prose in the *same* paragraph, e.g. `Within-structure alignment. AbstractExplorer also aligns...`) — that pattern is correct as-is and must not be touched.
