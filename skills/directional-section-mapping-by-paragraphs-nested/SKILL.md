...
Like every skill in the `directional-section-mapping` family, this is a **single-direction** pass: it finds, for every paper1 candidate, its single closest paper2 candidate or no match — it does not check the reverse direction and does not do bidirectional confirmation. Run it twice (swapping which paper is `paper1`) if both directions are wanted. The output contains exactly one entry per paper1 candidate; a candidate never maps to multiple paper2 candidates in one directional pass.
...
- **Choose one best counterpart or `null`.** Even when a paper1 candidate has partial correspondence with several paper2 candidates, return only the single strongest role correspondence across its complete evidence. Do not split one source candidate across multiple output entries. If no candidate is a defensible overall best match, return `null` rather than forcing a choice.
...
The array must contain exactly one output object for every paper1 candidate constructed in Step 1, in the same order as that candidate list.
...
- **Returning multiple matches for one paper1 candidate.** Choose the single best complete-role correspondence or `null`; partial matches must not create extra output entries.
